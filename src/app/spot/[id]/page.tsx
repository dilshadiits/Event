...
export default function SpotRegistrationPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        fetch(`/api/events?id=${id}`)
            ...
            .then(event => {
                if (event.id) {
                    ...
                    if (event.entryPassImage) setEntryPassImage(event.entryPassImage);
                }
    ...
    const generateSinglePass = useCallback(async (name: string, qrValue: string, isGuest: boolean, channel?: string): Promise<string> => {
        ...
        // plain white QR card instead of failing the whole registration flow.
